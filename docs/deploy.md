...
1. 每批一张表；单号贴 **A 列**  
2. 表头：`单号 | 状态 | 扫码时间 | 货品描述`  
3. **D 列货品描述（建议）**：与 A 列单号一一对应粘贴；**建议每格 ≤20 个汉字，或 ≤40 个字符**（取更严者作备表习惯），方便仓口色块一眼扫完。前端**不会**因超长而截断或拒挂载，但过长会撑高色块、拖慢余光阅读。  
4. 把该表链接填进对应 `batches[].sheetUrl`  
...
2. **下拉选择批次** → **挂载并开扫**（进入主屏并自动开摄像头；顶栏：表文件名 · 已收 x/y；底栏：停止 / 换批）  
3. 允许摄像头后对准条码  
4. **绿/橙色块**：主文案为货品描述（空则「（无货品描述）」），结果词半透明垫底，单号小字在下；**灰色块**：主文案为扫到的码，「不在清单」垫底。写表失败会红字并回滚；停止后可「继续扫码」  
5. 电脑筛空/`未收` = 漏收  
6. 全部收完：Apps Script → **停用** 网页应用  

摄像头仍失败时：确认不是 `http://192.168.…` / `file://`；需能访问 `unpkg.com`（html5-qrcode）。本轮**不**依赖 WASM polyfill CDN。
